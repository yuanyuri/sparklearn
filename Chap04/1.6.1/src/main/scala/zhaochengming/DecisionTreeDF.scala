...
object DecisionTreeDF extends App {
  ...
  val filePath = "F:\\for learn\\Spark_learn\\ch04-rdf\\dataset\\covtype.data"
  val dataWithOutHeader = sqlContext.read.format("com.databricks.spark.csv")
  ...
  /*决策树算法*/
  val runDecisionTree = new DecisionTreeDF(sqlContext)
  //1.简单决策树展示
...
class DecisionTreeDF(private val sqlContext:SQLContext){
  import sqlContext.implicits._
  /*0.取消原数据的独热编码，减小内存使用*/
  ...
  def simpleDecisionTree(trainData:DataFrame, testData:DataFrame, unOneHot:Boolean=false) : Unit = {
    ...
    val testAccuracy = evaluator.setMetricName("precision").evaluate(testPredictions)
